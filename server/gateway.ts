...
const RPC_CACHE_TIER: Record<string, CacheTier> = {
  ...
  '/api/signalmap/v1/list-signal-map-events': 'fast',

  // Entry required by tests/route-cache-tier.test.mjs.
...
function hasApiKey(request: Request): boolean {
  return Boolean(request.headers.get('X-WorldMonitor-Key') || request.headers.get('X-Api-Key'));
}

function hasRequestOrigin(request: Request): boolean {
...
function canUsePublicBrowserBypass(request: Request): boolean {
  return isSameOriginBrowserRequest(request) && !hasRequestOrigin(request) && !hasApiKey(request);
}
...
export function createDomainGateway(
  ...
  return async function handler(originalRequest: Request): Promise<Response> {
    ...
    // Product-tier RPC gates are disabled in this personal SignalMap fork. getRequiredTier()
    // returns null, so feature routes do not trigger bearer resolution.
    const isTierGated = getRequiredTier(pathname) !== null;

    // Session resolution is retained for compatibility if a future local route
    // explicitly opts into getRequiredTier().
    ...
    // API key validation. Product-tier feature routes do not force API keys.
    const forceApiKey = isTierGated && !sessionUserId;
    let keyCheck = (
      !forceApiKey && canUsePublicBrowserBypass(request)
        ? { valid: true, required: false }
        : validateApiKey(request, { forceKey: forceApiKey })
    ...
    if (keyCheck.required && !keyCheck.valid && wmKey.startsWith('wm_')) {
      ...
      if (userKeyResult) {
        ...
        // Inject x-user-id for downstream request context.
        if (!sessionUserId) {
    ...
    // Product-tier entitlement checks are disabled in this personal SignalMap fork.
    // checkEntitlement() is retained as a compatibility no-op.
