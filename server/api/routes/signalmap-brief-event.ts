...
const LOCK_TIMEOUT_SECONDS = () =>
  Number(process.env.SIGNALMAP_BRIEF_PER_EVENT_LOCK_TIMEOUT_SECONDS ?? 30);
...
async function defaultSynthesize(input: PerEventInput): Promise<BriefResult> {
  return synthesizePerEvent(input);
}

let synthesizePerEventBrief: (input: PerEventInput) => Promise<BriefResult> = defaultSynthesize;

export function setSynthesizePerEventBrief(fn: (input: PerEventInput) => Promise<BriefResult>): void {
...
export async function handleSignalMapBriefEvent(
  ...
  try {
    brief = await synthesizePerEventBrief(input);
  } catch (err) {
