...
const SIGNALMAP_PROVIDER_KEY = 'signalmap:providers:v1';
const SIGNALMAP_PROVIDER_META_KEY = 'seed-meta:signalmap:providers';
const SIGNALMAP_PROVIDER_TTL = 3600;
const SIGNALMAP_PROVIDER_META_TTL = 604800;
...
function buildSignalMapProviderHealth(statuses: ServiceStatus[], fetchedAtMs = Date.now()) {
  const counts: Record<string, number> = {};
  for (const status of statuses) {
    counts[status.status] = (counts[status.status] ?? 0) + 1;
  }
  const degradedProviders = statuses.filter((status) => (
    status.status !== 'SERVICE_OPERATIONAL_STATUS_OPERATIONAL' &&
    status.status !== 'SERVICE_OPERATIONAL_STATUS_MAINTENANCE'
  ));
  return {
    status: degradedProviders.length > 0 ? 'degraded' : 'ok',
  ...
  try {
    ...
    if (results) {
      ...
      if (source === 'fresh') {
        ...
        const signalMapHealth = buildSignalMapProviderHealth(results, fetchedAt);
        setCachedJson('seed-meta:infra:service-statuses', { fetchedAt, recordCount: results.length }, 604800).catch(() => {});
        Promise.all([
          setCachedJson(SIGNALMAP_PROVIDER_KEY, signalMapHealth, SIGNALMAP_PROVIDER_TTL, true),
          setCachedJson(
            SIGNALMAP_PROVIDER_META_KEY,
            {
              fetchedAt,
              recordCount: signalMapHealth.recordCount,
              sourceVersion: 'provider-status-health-v1',
              status: signalMapHealth.status,
            },
            SIGNALMAP_PROVIDER_META_TTL,
            true,
          ),
        ]).catch(() => {});
      }
