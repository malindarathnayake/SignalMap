...
export function resolveSignalMapDistillTimeoutMs(options = {}) {
...
function fallbackResult(input, fallbackReason, error) {
  return {
    status: 'fallback',
...
export function resolveSignalMapDistillBridgeConfig(options = {}) {
  ...
  const distillRoot = resolve(configuredRoot);
...
function normalizeDistilledArticle(output) {
  ...
  const sourceName = cleanString(output.sourceName);

  if (!title || !articleBody || !canonicalUrl || !SUPPORTED_SOURCE_NAMES.has(sourceName)) {
    return undefined;
  }
...
function withTimeout(promise, timeoutMs) {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => {
      const error = new Error(`Distill extraction timed out after ${timeoutMs}ms`);
...
export async function extractSignalMapArticleWithDistill(input, options = {}) {
  ...
  const descriptorPath = resolve(config.distillRoot, 'descriptors', descriptorFile);
