...
const SIGNALMAP_RADAR_KEY = 'signalmap:radar:v1';
const SIGNALMAP_RADAR_META_KEY = 'seed-meta:signalmap:radar';
...
const SIGNALMAP_RADAR_TTL = 3600;
...
function countActiveOutages(outagesPayload, now = Date.now()) {
  return Array.isArray(outagesPayload?.outages)
    ? outagesPayload.outages.filter((outage) => !outage.endedAt || outage.endedAt > now).length
    : 0;
}

function countActiveTrafficAnomalies(anomaliesPayload, now = Date.now()) {
  return Array.isArray(anomaliesPayload?.anomalies)
    ? anomaliesPayload.anomalies.filter((anomaly) => {
        const activeStatus = String(anomaly.status || '').toUpperCase() === 'ACTIVE';
        const notEnded = !anomaly.endDate || anomaly.endDate > now;
        return activeStatus || notEnded;
      }).length
    : 0;
}

async function writeSignalMapRadarHealth({
  outagesPayload,
  anomaliesPayload,
  degraded,
  ...
  const activeOutages = countActiveOutages(outagesPayload, fetchedAtMs);
  const activeTrafficAnomalies = countActiveTrafficAnomalies(anomaliesPayload, fetchedAtMs);
  const recordCount = activeOutages + activeTrafficAnomalies;
  const payload = {
    status: degraded ? 'degraded' : 'ok',
    fetchedAt,
    recordCount,
    activeOutages,
    activeTrafficAnomalies,
  };
...
async function fetchAll() {
  ...
  await writeSignalMapRadarHealth({
    outagesPayload: outagesResult.value,
    anomaliesPayload: anomalies,
    degraded: ddosResult.status === 'rejected' || anomaliesResult.status === 'rejected',
  });
