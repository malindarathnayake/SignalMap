...
].map((value) => String(value).toUpperCase()).filter(isIso2));

export const SIGNALMAP_STATIC_PLACES = [
...
const TEXTUAL_COUNTRY_TOKENS = new Map([
  ['u s', 'US'],
  ['u s a', 'US'],
  ['u k', 'GB'],
]);
...
function readConfidenceMin(options) {
...
function buildStaticPlaceIndex(places) {
  const byNameAndCountry = new Map();
...
export function countryBboxCentroid(bbox) {
...
function resolveKnownIso2(input, resolvers = COUNTRY_RESOLVERS) {
  const iso2 = resolveIso2(input, resolvers);
  return normalizeIso2(iso2);
}

export function resolveSignalMapCountryIso2(location, options = {}) {
...
function resolveExplicitCountryEvidence(location, resolvers) {
  return (
...
function resolveExplicitTextualCountryEvidence(location, resolvers) {
  return (
    resolveKnownIso2({ name: location?.countryName }, resolvers) ||
    resolveKnownIso2({ name: location?.country }, resolvers)
  );
...
function resolveEvidenceTextCountryIso2(location, resolvers) {
...
function resolveCountryEvidenceIso2(location, options = {}) {
  const resolvers = options.resolvers ?? COUNTRY_RESOLVERS;
  return resolveExplicitCountryEvidence(location, resolvers) ||
    resolveEvidenceTextCountryIso2(location, resolvers);
}

function resolveTextualCountryEvidenceIso2(location, options = {}) {
  const resolvers = options.resolvers ?? COUNTRY_RESOLVERS;
  return resolveExplicitTextualCountryEvidence(location, resolvers) ||
    resolveEvidenceTextCountryIso2(location, resolvers);
}

function findStaticPlace(location, countryIso2, options = {}) {
  const places = options.staticPlaces ?? SIGNALMAP_STATIC_PLACES;
  const index = places === SIGNALMAP_STATIC_PLACES ? STATIC_PLACE_INDEX : buildStaticPlaceIndex(places);
...
function markerEligibleFor(location, confidenceMin) {
  return finiteNumber(location.lat) &&
    finiteNumber(location.lon) &&
    finiteNumber(location.confidence) &&
    location.confidence >= confidenceMin;
}

function unresolvedLocation(location, geocodeStatus, extra = {}) {
...
export function resolveSignalMapLocation(location, options = {}) {
  ...
  const countryEvidenceIso2 = textualCountryEvidenceIso2 || resolveCountryEvidenceIso2(location, options);
  ...
  const nameCountryIso2 = resolveKnownIso2({ name: location?.name }, options.resolvers ?? COUNTRY_RESOLVERS);
  const countryOnly = location?.scope === 'country' || (nameCountryIso2 && nameCountryIso2 === countryIso2);
...
export function resolveSignalMapLocations(locations, options = {}) {
