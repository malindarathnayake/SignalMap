...
} from './signalmap-embedding-model.mjs';
...
function uniqueCountryIso2(values, limit = COUNTRY_ISO2_MAX_COUNT) {
  const seen = new Set();
  const result = [];
  for (const value of Array.isArray(values) ? values : []) {
...
function normalizeLocations(locations) {
  ...
  for (const location of locations) {
    ...
    normalized.push({
      ...
      ...(Number.isFinite(Number(location.confidence)) ? { confidence: Number(location.confidence) } : {}),
...
function vectorFailure(reason, message) {
...
function validateVectorDim(vector, dim) {
...
function degradedVectorStore(config, reason, extra = {}) {
  return {
    status: 'degraded',
    enabled: true,
...
async function validateExistingTableSchema(table) {
  ...
  const missingFields = REQUIRED_SIGNALMAP_VECTOR_FIELDS.filter((fieldName) => !fieldNames.has(fieldName));
...
function publicRelatedRecord(result) {
...
function serializeVectorRecordForLanceDb(record) {
  const serialized = {
    ...record,
...
export function resolveSignalMapVectorStoreConfig(env = process.env) {
  ...
  return {
    ...
    minScore: parseNonNegativeNumber(env?.SIGNALMAP_VECTOR_MIN_SCORE, DEFAULT_SIGNALMAP_VECTOR_MIN_SCORE),
    embeddingModel: embedding.model,
    embeddingDim: embedding.dim,
  };
}

export function createSignalMapVectorRecord(event, vector, options = {}) {
  const config = {
  ...
  if (!vectorCheck.ok) {
    throw vectorFailure(vectorCheck.reason, `Expected vector length ${vectorCheck.expectedDim}, got ${vectorCheck.actualDim}`);
  }

  const title = boundedString(event?.canonicalTitle, TITLE_MAX_CHARS) ??
  ...
  const locationNames = uniqueStrings(locations.map((location) => location.name), LOCATION_MAX_COUNT);
  ...
  const contentHash = cleanString(event?.contentHash) ??
    hashValue([title, summary, canonicalUrl, publishedAt].join('\n'));
  const sourceTextHash = cleanString(event?.sourceTextHash) ??
    hashValue([canonicalUrl, sourceName, title, summary].join('\n'));
  const eventId = cleanString(event?.eventId) ?? cleanString(event?.id) ?? hashValue(`${canonicalUrl}\n${title}`).slice(0, 24);
  ...
  return {
    ...
    contentHash,
    sourceTextHash,
    evidenceSnippetsJson: JSON.stringify(locations.map((location) => location.evidence).filter(Boolean)),
...
async function resolveLanceDbModule(options) {
...
async function openExistingTable(db, tableName) {
  try {
...
export async function openVectorStore(options = {}) {
  ...
  if (options.table) {
    ...
    if (!schemaCheck.ok) {
      return degradedVectorStore(config, schemaCheck.reason, {
...
export async function upsertStoryVector(store, recordOrEvent, vector, options = {}) {
  ...
  const dim = options.embeddingDim ?? store.config?.embeddingDim ?? DEFAULT_SIGNALMAP_EMBEDDING_DIM;
  ...
  const record = recordOrEvent?.embeddingDim && recordOrEvent?.vector
...
export async function findRelatedStories(store, vector, options = {}) {
  ...
  const searchPromise = (async () => {
    ...
    return rows
      ...
      .filter((row) => row.score === undefined || row.score >= config.minScore)
...
export async function pruneOldVectors(store, options = {}) {
...
async function resolveRecordCount(table) {
...
export async function getVectorStoreHealth(storeOrOptions = {}) {
  ...
  return {
    ...
    ...(store?.errorClass || countErrorClass ? { errorClass: store?.errorClass ?? countErrorClass } : {}),
