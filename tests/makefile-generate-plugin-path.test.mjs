...
function extractGenerateRecipe() {
  ...
  const start = lines.findIndex((line) => /^generate:/.test(line));
  if (start === -1) throw new Error('generate target not found in Makefile');
...
describe('Makefile generate target — plugin path resolution', () => {
  ...
  test('statically defines the GOBIN-or-GOPATH/bin plugin directory expression', () => {
    // This intentionally avoids shelling out to bash/go. The test only
    // needs to prove the Makefile recipe still pins plugin lookup to
    // GOBIN, or GOPATH/bin when GOBIN is empty.
    assert.match(
      recipe,
      /PLUGIN_DIR=\$\$\(gobin=\$\$\(go env GOBIN\);\s*if \[ -n "\$\$gobin" \]; then printf '%s' "\$\$gobin"; else printf '%s\/bin' "\$\$\(go env GOPATH \| cut -d: -f1\)"; fi\)/,
      'generate recipe must compute PLUGIN_DIR from GOBIN, falling back to the first GOPATH entry plus /bin',
    );
    assert.match(
      recipe,
      /PATH="\$\$PLUGIN_DIR:\$\$PATH"\s+"\$\$BUF_BIN" generate/,
      'generate recipe must prepend PLUGIN_DIR to PATH only for "$BUF_BIN" generate',
    );
  });
});
