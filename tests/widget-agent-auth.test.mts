...
describe('widget-agent unified tester key auth', () => {
  ...
  it('accepts X-WorldMonitor-Key and preserves basic relay requests', async () => {
    ...
    assert.equal(headers.get('X-Pro-Key'), null);
  ...
  it('accepts X-WorldMonitor-Key for pro relay requests', async () => {
    const res = await handler(new Request('https://www.worldmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.worldmonitor.app',
  ...
  it('rejects invalid tier values before relay fetch', async () => {
    const res = await handler(new Request('https://www.worldmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.worldmonitor.app',
      ...
      body: JSON.stringify({ prompt: 'Build a widget', mode: 'create', tier: 'claude-opus' }),
    }));
    ...
    const body = await res.json() as { error: string };
    assert.equal(body.error, 'Invalid tier value');
  });
});
