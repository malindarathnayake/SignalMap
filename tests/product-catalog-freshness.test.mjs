...
const PRODUCT_ID_SOURCE_EXTENSIONS = new Set(['.ts', '.tsx', '.mjs', '.js']);
const PRODUCT_ID_ALLOWED_PATH_PARTS = [
  'node_modules',
  '.claude/worktrees/',
  'convex/_generated/',
...
function isAllowedProductIdPath(path) {
  return PRODUCT_ID_ALLOWED_PATH_PARTS.some(part => path === part || path.includes(part));
}

function isAllowedProductIdDir(path) {
  return PRODUCT_ID_ALLOWED_DIRS.some(dir => path === dir || path.startsWith(`${dir}/`));
}

function findRawProductIdViolations() {
  const violations = [];

  function walk(dir) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      const absPath = join(dir, entry.name);
      const path = repoPath(absPath);

      if (entry.isDirectory()) {
        if (!isAllowedProductIdDir(path)) walk(absPath);
        continue;
      }

      if (!entry.isFile()) continue;
      if (!PRODUCT_ID_SOURCE_EXTENSIONS.has(extname(entry.name))) continue;
      if (isAllowedProductIdPath(path)) continue;

      const lines = readFileSync(absPath, 'utf8').split(/\r?\n/);
      lines.forEach((line, idx) => {
        if (line.includes('pdt_')) {
          violations.push(`${path}:${idx + 1}:${line}`);
        }
      });
    }
  }
...
describe('Product ID guard', () => {
  it('no raw pdt_ strings outside allowed paths', () => {
    ...
    const result = findRawProductIdViolations().join('\n');
