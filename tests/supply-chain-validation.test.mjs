 ...
 * Locks in the "400 on bad input / empty-200 on no data" contract after
 * koala73 review HIGH(new) #2 on #3242.
 *
 ...
 * genuine "no data for this country" path.
 *
 * Fix: input-shape errors throw ValidationError (sebuf -> HTTP 400).
 * Product-tier RPC gates are disabled in this fork, so unkeyed callers reach
 * normal feature execution and fall back to empty-200 only when data is absent.
 */
...
describe('supply-chain handlers: input-shape validation returns 400, not empty-200', () => {
  ...
  describe('getCountryProducts', () => {
    ...
    it('unkeyed callers reach normal no-data fallback on a well-formed iso2', async () => {
  ...
  describe('getMultiSectorCostShock', () => {
    ...
    it('unkeyed callers reach normal no-data fallback on valid inputs', async () => {
      ...
      assert.match(res.unavailableReason, /No seeded import data/);
    });
  });
});
