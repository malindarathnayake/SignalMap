 ...
 * - get-bypass-options handler: feature enrichment is not suppressed by product tier
 * - get-country-cost-shock handler: feature enrichment is not suppressed by product tier
 * - gateway.ts: new slow-browser entries for both RPCs
 * - premium-paths.ts: product-tier RPC gates remain disabled
...
describe('get-bypass-options handler source code', () => {
  ...
  it('keeps the premium compatibility helper, which allows feature execution in this fork', async () => {
    ...
    assert.equal(await isCallerPremium(new Request('https://worldmonitor.app/api/supply-chain/v1/get-bypass-options')), true);
  });
...
describe('get-country-cost-shock handler source code', () => {
  ...
  it('keeps the premium compatibility helper, which allows feature execution in this fork', async () => {
    assert.match(src, /isCallerPremium/);
    assert.equal(await isCallerPremium(new Request('https://worldmonitor.app/api/supply-chain/v1/get-country-cost-shock')), true);
  });
...
// ========================================================================
// 5. Gateway: both RPCs keep explicit cache-tier entries
// ========================================================================

describe('Gateway cache tier registration', () => {
...
// ========================================================================
// 6. Product-tier premium paths disabled
// ========================================================================

describe('Product-tier premium paths disabled', () => {
  const src = readSrc('src/shared/premium-paths.ts');

  it('get-bypass-options is not product-tier gated', () => {
    assert.doesNotMatch(src, /\/api\/supply-chain\/v1\/get-bypass-options/);
  });

  it('get-country-cost-shock is not product-tier gated', () => {
    assert.doesNotMatch(src, /\/api\/supply-chain\/v1\/get-country-cost-shock/);
  });

  it('get-sector-dependency is not product-tier gated', () => {
