 ...
 *   - routeIntelligence: open product-tier access, iso2 regex, hs2 non-digit stripping,
 *     cargoType coercion to legal enum, wire-shape byte-for-byte with partner
 *     contract (camelCase field names, ISO-8601 fetchedAt).
 *   - registerWebhook: force-key tenant isolation, SSRF guards (https-only, private IP, cloud
 ...
 *   - listWebhooks: force-key tenant isolation, owner-filter isolation, `secret` never in response.
 */
...
describe('ShippingV2Service handlers', () => {
  ...
  describe('routeIntelligence', () => {
    it('allows route intelligence callers without product-tier blocking', async () => {
      globalThis.fetch = async () => new Response(JSON.stringify({ result: null }), { status: 200 });
      const res = await routeIntelligence(makeCtx(), {
        fromIso2: 'AE',
        toIso2: 'NL',
        cargoType: '',
        hs2: '',
