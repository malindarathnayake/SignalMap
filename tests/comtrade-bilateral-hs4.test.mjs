...
describe('getCountryProducts sebuf handler (server/worldmonitor/supply-chain/v1/get-country-products.ts)', () => {
  ...
  it('keeps the premium compatibility helper against ctx.request', async () => {
    assert.ok(
      src.includes('isCallerPremium'),
      'must keep isCallerPremium compatibility call',
    );
    assert.ok(
      src.includes('isCallerPremium(ctx.request)'),
      'must invoke isCallerPremium(ctx.request) so the sebuf gateway request context is preserved',
    );
    assert.equal(await isCallerPremium(new Request('https://worldmonitor.app/api/supply-chain/v1/get-country-products')), true);
  });

  it('declares the typed empty payload used for no-data fallbacks after validation', () => {
    assert.ok(
    ...
    assert.ok(validIdx !== -1, 'must validate iso2 before cache lookup');
  });
...
describe('CountryDeepDivePanel product imports section', () => {
  ...
  it('product imports card initializes with loading state and no client product-tier guard', () => {
    assert.doesNotMatch(src, /import\s+\{\s*hasPremiumAccess\s*\}/);
    assert.doesNotMatch(src, /import\s+\{\s*getAuthState\s*\}/);
    assert.doesNotMatch(src, /import\s+\{\s*trackGateHit\s*\}/);
    assert.doesNotMatch(src, /hasPremiumAccess\(getAuthState\(\)\)/);
    assert.doesNotMatch(src, /makeProLocked/);
    assert.doesNotMatch(src, /Upgrade to PRO|Bypass corridors available with PRO/);
    ...
    const productImportsBlock = src.slice(productImportsIdx, src.indexOf('const [debtCard', productImportsIdx));
    assert.match(
      productImportsBlock,
      /productImportsCardBody\.append\(this\.makeLoading\('Loading product data\\u2026'\)\)/,
      'CountryDeepDivePanel: product imports card must use the loading path directly',
    );
    assert.doesNotMatch(productImportsBlock, /isPro|makeProLocked|Upgrade to PRO/);
  });

  it('uses textContent for product rendering (XSS-safe, no innerHTML)', () => {
    const renderBody = extractBody(src, 'private renderProductDetail');
    assert.ok(
  ...
  it('resetPanelContent clears productImportsBody', () => {
    const resetBody = extractBody(src, 'private resetPanelContent');
    assert.ok(
