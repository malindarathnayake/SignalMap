...
function findMatchingBrace(source, openIndex) {
  ...
  for (let i = openIndex; i < source.length; i++) {
    ...
    if (char === '"' || char === "'" || char === '`') {
      const quotedEnd = skipQuoted(source, i, char);
...
function extractMethodBody(source, signaturePattern, label) {
  const signature = signaturePattern.exec(source);
  assert.ok(signature, `could not locate ${label} signature`);
  const openOffset = signature[0].lastIndexOf('{');
  assert.ok(openOffset >= 0, `could not locate ${label} opening brace`);
  const openIndex = signature.index + openOffset;
  const closeIndex = findMatchingBrace(source, openIndex);
  assert.ok(closeIndex >= 0, `could not locate ${label} closing brace`);
...
describe('webmcp App.ts binding: readiness + teardown', () => {
  ...
  it('is called before the first await in init()', () => {
    // Use a balanced-brace scan so comments or member spacing after init()
    // do not affect the source-level contract check.
    const initBody = extractMethodBody(
      appSrc,
      /^\s*public\s+async\s+init\(\):\s*Promise<void>\s*\{/m,
      'init() body',
    );
    const preAwait = initBody.split(/\n\s+await\s/, 2)[0];
    assert.match(
      preAwait,
      /registerWebMcpTools\(/,
  ...
  it('destroy() aborts the WebMCP controller so re-inits do not duplicate registrations', () => {
    // Use the same balanced-brace scan as init() so trailing comments or
    // member spacing do not affect teardown extraction.
    const destroyBody = extractMethodBody(
      appSrc,
      /^\s*public\s+destroy\(\):\s*void\s*\{/m,
      'destroy() body',
    );
    assert.match(
      destroyBody,
