 ...
 *   4. Product-tier RPC path gate disabled.
 *   5. CountryDeepDivePanel surface: ungated card, slider, debounced re-fetch, reset.
 */
...
describe('getMultiSectorCostShock sebuf handler', () => {
  ...
  it('keeps the premium compatibility helper, which allows feature execution in this fork', async () => {
    assert.match(src, /isCallerPremium\(ctx\.request\)/);
    assert.equal(await isCallerPremium(new Request('https://worldmonitor.app/api/supply-chain/v1/get-multi-sector-cost-shock')), true);
  });
...
// ========================================================================
// 4. Premium paths: get-multi-sector-cost-shock is not product-tier gated.
// ========================================================================
...
describe('CountryDeepDivePanel Cost Shock Calculator', () => {
  ...
  it('initializes the card with loading state and no client product-tier guard', () => {
    assert.doesNotMatch(src, /hasPremiumAccess\(getAuthState\(\)\)/);
    assert.doesNotMatch(src, /makeProLocked/);
    assert.doesNotMatch(src, /Upgrade to PRO/);

    const cardIdx = src.indexOf('const [costShockCalcCard');
    assert.notEqual(cardIdx, -1, 'CountryDeepDivePanel: must declare costShockCalcCard');
    const cardBlock = src.slice(cardIdx, src.indexOf('const [productImportsCard', cardIdx));
    assert.match(
      cardBlock,
      /costShockCalcBody\.append\(this\.makeLoading\('Loading cost shock calculator\\u2026'\)\)/,
      'CountryDeepDivePanel: cost shock calculator must use the loading path directly',
    );
    assert.doesNotMatch(cardBlock, /isPro|makeProLocked|Upgrade to PRO/);
  });
