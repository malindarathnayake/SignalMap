 ...
 *     backpressure, premium compatibility helper, AbortSignal.timeout on Redis fetches.
 *   - get-scenario-status: JOB_ID_RE path-traversal guard, premium compatibility helper.
...
const runScenarioSrc = readFileSync(join(root, 'server/worldmonitor/scenario/v1/run-scenario.ts'), 'utf8');
const getScenarioStatusSrc = readFileSync(join(root, 'server/worldmonitor/scenario/v1/get-scenario-status.ts'), 'utf8');
...
describe('ScenarioService handlers', () => {
  ...
  describe('runScenario', () => {
    it('keeps the premium compatibility helper open for this fork', async () => {
      assert.match(
        runScenarioSrc,
        /isCallerPremium\(ctx\.request\)/,
        'runScenario must keep invoking isCallerPremium(ctx.request) for compatibility',
      );
      assert.equal(
        await isCallerPremium(new Request('https://worldmonitor.app/api/scenario/v1/run-scenario')),
        true,
      );
    });
  ...
  describe('getScenarioStatus', () => {
    it('keeps the premium compatibility helper open for this fork', async () => {
      assert.match(
        getScenarioStatusSrc,
        /isCallerPremium\(ctx\.request\)/,
        'getScenarioStatus must keep invoking isCallerPremium(ctx.request) for compatibility',
      );
      assert.equal(
        await isCallerPremium(new Request('https://worldmonitor.app/api/scenario/v1/get-scenario-status')),
        true,
      );
    });
