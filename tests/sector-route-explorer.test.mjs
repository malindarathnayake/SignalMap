...
describe('CountryDeepDivePanel sector route interaction', () => {
  ...
  it('keeps sector route bypass UI free of product gates', () => {
    assert.doesNotMatch(
      deepDiveSrc,
      /import\s+\{\s*trackGateHit\s*\}/,
      'CountryDeepDivePanel must not import trackGateHit for sector route bypass UI',
    );
    assert.doesNotMatch(
      deepDiveSrc,
      /\btrackGateHit\s*\(/,
      'CountryDeepDivePanel must not call trackGateHit for sector route bypass UI',
    );
    assert.doesNotMatch(
      deepDiveSrc,
      /Bypass corridors available with PRO/,
      'CountryDeepDivePanel must not render the stale PRO bypass gate',
    );
    assert.doesNotMatch(
      deepDiveSrc,
      /\bmakeProLocked\b/,
      'CountryDeepDivePanel must not use makeProLocked for sector route bypass UI',
    );
  });
