...
function getRedisClient() {
  ...
  // Prevent 'Unhandled error event' from crashing the runtime on transient
  // connection issues; ioredis retries with backoff in the background.
  _redisClient.on('error', (err) => {
    console.warn('[bootstrap] redis client error:', err && err.message ? err.message : err);
  });
  return _redisClient;
}
...
function hasApiKey(req) {
  return Boolean(req.headers.get('X-WorldMonitor-Key') || req.headers.get('X-Api-Key'));
}

function hasRequestOrigin(req) {
...
function canUsePublicBrowserBypass(req) {
  return isSameOriginBrowserRequest(req) && !hasRequestOrigin(req) && !hasApiKey(req);
}
...
async function getCachedJsonBatch(keys) {
  ...
  try {
    ...
    console.warn('[bootstrap] redis pipeline failed:', err && err.message ? err.message : err);
    return result;
  }
  if (!pipelineResults) return result;

  for (let i = 0; i < keys.length; i++) {
    // ioredis pipeline.exec() returns Array<[Error|null, unknown]>
    const tuple = pipelineResults[i];
    if (!tuple || tuple[0]) continue; // skip command errors
    const raw = tuple[1];
...
export default async function handler(req) {
  ...
  if (!canUsePublicBrowserBypass(req)) {
