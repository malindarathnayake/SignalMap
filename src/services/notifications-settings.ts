...
export function renderNotificationsSettings(host: NotificationsSettingsHost): NotificationsSettingsResult {
  const isSignedIn = !!host.isSignedIn;
  ...
  if (isSignedIn) {
    ...
    html += `<div class="wm-pref-group-content wm-notif-tab-content">`;
    html += `<div class="ai-flow-toggle-desc">Sign in to configure Telegram, Slack, Discord, Email, webhook, and browser push alerts for your account.</div>`;
    html += `<button type="button" class="us-notif-ch-btn us-notif-ch-btn-primary" id="usNotifSignInBtn">Sign in</button>`;
    html += `</div>`;
  }
        ...
        if (signInBtn) {
          signInBtn.addEventListener('click', () => {
            import('@/services/clerk').then(m => m.openSignIn()).catch(() => {
              window.open('https://worldmonitor.app/sign-in', '_blank');
            });
          }, { signal });
        }
        return () => ac.abort();
