...
import {
  ...
  dispatchSignalMapState,
  fetchSignalMapState,
  isSignalMapVariant,
} from '@/services/signalmap';
import { SIGNALMAP_STORAGE_KEYS } from '@/config/signalmap';
...
export class DataLoaderManager implements AppModule {
  ...
  private boundSignalMapWatchlistHandler: (() => void) | null = null;
  private boundSignalMapStorageHandler: ((event: StorageEvent) => void) | null = null;
  ...
  init(): void {
    if (!isSignalMapVariant(SITE_VARIANT)) {
    ...
    if (isSignalMapVariant(SITE_VARIANT)) {
      this.boundSignalMapWatchlistHandler = () => {
        void this.loadSignalMap();
      };
      this.boundSignalMapStorageHandler = (event: StorageEvent) => {
        if (
          event.key === SIGNALMAP_STORAGE_KEYS.watchRegions ||
          event.key === SIGNALMAP_STORAGE_KEYS.watchProviders
        ) {
          void this.loadSignalMap();
        }
      };
      window.addEventListener(
        SIGNALMAP_WATCHLIST_CHANGED_EVENT,
        this.boundSignalMapWatchlistHandler as EventListener,
      );
      window.addEventListener('storage', this.boundSignalMapStorageHandler);
    }
  ...
  destroy(): void {
    ...
    if (this.boundSignalMapWatchlistHandler) {
      window.removeEventListener(
        SIGNALMAP_WATCHLIST_CHANGED_EVENT,
        this.boundSignalMapWatchlistHandler as EventListener,
      );
      this.boundSignalMapWatchlistHandler = null;
    }
    if (this.boundSignalMapStorageHandler) {
      window.removeEventListener('storage', this.boundSignalMapStorageHandler);
      this.boundSignalMapStorageHandler = null;
    }
  ...
  async loadAllData(forceAll = false): Promise<void> {
    ...
    if (isSignalMapVariant(SITE_VARIANT)) {
      await runGuarded('signalmap', () => this.loadSignalMap());
      return;
    }
  ...
  async loadSignalMap(): Promise<void> {
    if (!isSignalMapVariant(SITE_VARIANT) || this.ctx.isDestroyed) return;

    const controller = new AbortController();
    const timeoutId = globalThis.setTimeout(() => controller.abort(), 12_000);
    const state = await fetchSignalMapState({
      timeRange: this.ctx.currentTimeRange,
      signal: controller.signal,
    }).finally(() => globalThis.clearTimeout(timeoutId));
    dispatchSignalMapState(state);
    this.ctx.map?.setSignalMapEvents(state.events);

    const payload = {
      events: state.events,
      sourceHealth: state.sourceHealth,
      stale: state.stale,
      upstreamUnavailable: state.upstreamUnavailable,
      fetchedAt: state.fetchedAt,
      watchlist: state.watchlist,
    };

    this.callPanel('signalmap', 'setData', payload);
    this.callPanel('signalmap-status-strips', 'setData', payload);
    this.callPanel('signalmap-feed', 'setEvents', payload);
    this.callPanel('signalmap-inspector', 'setState', payload);
  }
