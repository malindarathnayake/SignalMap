...
/** Legacy premium panel registry. Public SignalMap gating resolves to NONE. */
...
export class PanelLayoutManager implements AppModule {
  ...
  constructor(ctx: AppContext, callbacks: PanelLayoutManagerCallbacks) {
    ...
    this.unsubscribeEntitlementChange = onEntitlementChange(() => {
      ...
      // Re-run panel gating on every entitlement snapshot so stale legacy
      // locks are cleared after public SignalMap access resolves to NONE.
  ...
  /** Reactively clear or apply legacy panel gating based on auth state. */
  ...
  private createPanels(): void {
    ...
    // Legacy panel gating state is reconciled by updatePanelGating().
    ...
    // magazine on click. Self-fetching, with source errors rendered
    // inside the panel instead of as product gates.
    this.createPanel('latest-brief', () => new LatestBriefPanel());
    const signalMapShell = this.createPanel('signalmap', () => new SignalMapShell());
    if (signalMapShell) {
      this.ctx.panels['signalmap-status-strips'] = signalMapShell;
      this.ctx.panels['signalmap-feed'] = signalMapShell;
      this.ctx.panels['signalmap-inspector'] = signalMapShell;
    }
    ...
    if (import.meta.env.DEV) {
      ...
      const signalMapAliases = new Set(['signalmap-status-strips', 'signalmap-feed', 'signalmap-inspector']);
      const extra = [...created].filter(k => !configured.has(k) && !signalMapAliases.has(k) && k !== 'runtime-config' && !k.startsWith('cw-') && !k.startsWith('mcp-'));
