...
export class CountryIntelManager implements AppModule {
  ...
  async openCountryBriefByCode(code: string, country: string, opts?: { maximize?: boolean }): Promise<void> {
    ...
    this.fetchFeatureSections(code);
    ...
    try {
      ...
      if (briefText) {
        ...
        if (fallbackBrief) {
          ...
          if (signals.radiationAnomalies > 0) lines.push(`?? Radiation anomalies: ${signals.radiationAnomalies}`);
          if (signals.temporalAnomalies > 0) lines.push(`?? Temporal anomaly alerts: ${signals.temporalAnomalies}`);
          if (signals.thermalEscalations > 0) lines.push(`??? Thermal escalation clusters: ${signals.thermalEscalations}`);
  ...
  private fetchFeatureSections(code: string): void {
    const rpcBase = getRpcBaseUrl();
    const economicClient = new EconomicServiceClient(rpcBase);
    const intelligenceClient = new IntelligenceServiceClient(rpcBase);
    const tradeClient = new TradeServiceClient(rpcBase);
    ...
    intelligenceClient.getCountryRisk({ countryCode: code }).then(resp => {
      if (this.ctx.countryBriefPage?.getCode() !== code) return;
