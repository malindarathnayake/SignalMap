...
interface SignalMapMarker extends BaseMarker {
  _kind: 'signalMap';
  id: string;
  title: string;
  severity: SignalMapEvent['severity'];
...
type GlobeMarker =
  ...
  | NewsLocationMarker | SignalMapMarker | FlashMarker | SatelliteMarker | SatFootprintMarker | ImagerySceneMarker
...
export class GlobeMap {
  ...
  private signalMapMarkers: SignalMapMarker[] = [];
  ...
  private buildMarkerElement(d: GlobeMarker): HTMLElement {
    ...
    if (d._kind === 'conflict') {
      ...
      el.title = d.title;
    } else if (d._kind === 'signalMap') {
      const sc = d.severity === 'critical' ? '#ef4444'
        : d.severity === 'high' ? '#f97316'
        : d.severity === 'medium' ? '#eab308'
        : d.severity === 'low' ? '#22c55e'
        : '#3b82f6';
      const watchRing = d.watchlistMatch
        ? `<div style="position:absolute;inset:-6px;border-radius:50%;border:2px solid #fcd34d99;${this.pulseStyle('2s')}"></div>`
        : '';
      el.innerHTML = GlobeMap.wrapHit(`
        <div style="position:relative;width:${d.watchlistMatch ? 14 : 11}px;height:${d.watchlistMatch ? 14 : 11}px;">
          ${watchRing}
          <div style="position:absolute;inset:0;border-radius:50%;background:${sc}cc;border:1.5px solid rgba(255,255,255,0.75);box-shadow:0 0 7px 2px ${sc}77;"></div>
        </div>`);
      el.title = `${d.title} (${d.severity})${d.watchlistMatch ? ' - Watchlist match' : ''}`;
  ...
  private flushMarkersImmediate(): void {
    ...
    markers.push(...this.signalMapMarkers);
  ...
  public setSignalMapEvents(events: SignalMapEvent[]): void {
    this.signalMapMarkers = (events ?? [])
      .filter((event) => event.markerEligible)
      .map((event) => {
      ...
      .filter((marker): marker is SignalMapMarker => marker !== null);
    this.flushMarkers();
  }
