...
import { getLiveStreamsAlwaysOn, setLiveStreamsAlwaysOn, subscribeLiveStreamsSettingsChange } from '@/services/live-stream-settings';
...
export interface LiveChannel {
  ...
  isLive?: boolean;
  hlsUrl?: string; // HLS manifest URL for native <video> playback
  embedUrl?: string; // Non-YouTube browser embed, e.g. Owncast /embed/video
...
export class LiveNewsPanel extends Panel {
  ...
  private idleModeBtn: HTMLButtonElement | null = null;
  ...
  private customEmbedIframe: HTMLIFrameElement | null = null;
  ...
  private getCustomEmbedUrl(channel: LiveChannel = this.activeChannel): string | undefined {
    if (!channel.embedUrl) return undefined;
    try {
      const url = new URL(channel.embedUrl);
  ...
  private destroyPlayer(): void {
    ...
    if (this.customEmbedIframe) {
      this.customEmbedIframe.src = 'about:blank';
      this.customEmbedIframe = null;
    }
  ...
  private createIdleModeButton(): void {
    this.idleModeBtn = document.createElement('button');
    this.idleModeBtn.className = 'live-mute-btn';
    this.updateIdleModeButton();
    this.idleModeBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      setLiveStreamsAlwaysOn(!this.alwaysOn);
    });
  }

  private updateIdleModeButton(): void {
    if (!this.idleModeBtn) return;
    this.idleModeBtn.classList.toggle('live-idle-always', this.alwaysOn);
    this.idleModeBtn.classList.toggle('live-idle-eco', !this.alwaysOn);
    this.idleModeBtn.title = this.alwaysOn
      ? t('components.insights.streamAlwaysOnToggle')
      : t('components.insights.streamEcoToggle');
    this.idleModeBtn.setAttribute('aria-label', this.idleModeBtn.title);
    this.idleModeBtn.setAttribute('aria-pressed', String(this.alwaysOn));
    this.idleModeBtn.innerHTML = this.alwaysOn
      ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4.9 19.1a10 10 0 0 1 0-14.2"/><path d="M19.1 4.9a10 10 0 0 1 0 14.2"/><path d="M8.5 15.5a5 5 0 0 1 0-7"/><path d="M15.5 8.5a5 5 0 0 1 0 7"/><circle cx="12" cy="12" r="1.5"/></svg>'
  ...
  private togglePlayback(): void {
    ...
    this.updateLiveIndicator();
    if (this.isPlaying && !this.player && !this.desktopEmbedIframe && !this.nativeVideoElement && !this.customEmbedIframe) {
  ...
  private createMuteButton(): void {
    ...
    if (this.idleModeBtn) header?.appendChild(this.idleModeBtn);
  ...
  private async resolveChannelVideo(channel: LiveChannel, forceFallback = false): Promise<void> {
    ...
    if (this.getCustomEmbedUrl(channel)) {
      channel.videoId = channel.fallbackVideoId;
      channel.isLive = true;
      return;
    }
  ...
  private renderCustomEmbedPlayer(): void {
    const embedUrl = this.getCustomEmbedUrl();
    if (!embedUrl) {
  ...
  private async initializePlayer(): Promise<void> {
    ...
    if (this.getCustomEmbedUrl(this.activeChannel)) {
      this.renderCustomEmbedPlayer();
      return;
    }
