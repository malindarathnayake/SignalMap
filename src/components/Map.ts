...
export class MapComponent {
  ...
  private renderOverlays(projection: d3.GeoProjection): void {
    ...
    if (this.signalMapEvents.length > 0) {
      ...
      this.signalMapEvents.forEach((event) => {
        if (!event.markerEligible) return;
        ...
        const pos = projection([location.lon, location.lat]);
        if (!pos || !Number.isFinite(pos[0]) || !Number.isFinite(pos[1])) return;

        const div = document.createElement('div');
        const size = event.watchlistMatch ? 14 : 11;
        div.className = `signalmap-svg-marker severity-${event.severity}${event.watchlistMatch ? ' watchlist-match' : ''}`;
        ...
        div.style.background = colors[event.severity] ?? '#3b82f6';
        div.style.border = event.watchlistMatch ? '2px solid #fef3c7' : '1px solid rgba(15, 23, 42, 0.85)';
        div.style.boxShadow = event.watchlistMatch
          ? '0 0 0 5px rgba(245, 158, 11, 0.22), 0 0 18px rgba(245, 158, 11, 0.45)'
          : '0 0 10px rgba(14, 165, 233, 0.28)';
        div.style.pointerEvents = 'auto';
        div.title = `${event.title} (${event.severity})`;
        this.overlays.appendChild(div);
      });
    }
