...
export class DeckGLMap {
  ...
  private signalMapEvents: SignalMapEvent[] = [];
  ...
  private buildLayers(): LayersList {
    ...
    // SignalMap and news geo-locations are always shown if data exists.
    if (this.signalMapEvents.length > 0) {
      layers.push(...this.createSignalMapEventsLayer());
    }
  ...
  private createSignalMapEventsLayer(): ScatterplotLayer[] {
    const markerEvents = this.signalMapEvents.filter((event) =>
      event.markerEligible && event.locations.some((location) =>
    ...
    const getPosition = (event: SignalMapEvent): [number, number] => {
      const location = event.locations.find((candidate) =>
    ...
    const watchlistEvents = markerEvents.filter((event) => event.watchlistMatch);
    const layers: ScatterplotLayer[] = [];
    if (watchlistEvents.length > 0) {
      layers.push(new ScatterplotLayer<SignalMapEvent>({
        id: 'signalmap-watchlist-halo-layer',
    ...
    layers.push(new ScatterplotLayer<SignalMapEvent>({
      id: 'signalmap-events-layer',
      data: markerEvents,
      getPosition,
      getRadius: (event) => event.watchlistMatch ? 18000 : 14000,
      getFillColor: (event) => {
        const rgb = SEVERITY_RGB[event.severity] ?? [59, 130, 246];
        return [...rgb, event.watchlistMatch ? 235 : 185] as [number, number, number, number];
      },
      getLineColor: (event) => event.watchlistMatch ? [255, 255, 255, 230] : [15, 23, 42, 190],
      lineWidthMinPixels: 1.5,
      radiusMinPixels: 5,
      radiusMaxPixels: 14,
      stroked: true,
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'signalmap-events-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.title)}</strong><br/>${text(obj.severity)} SignalMap event${obj.watchlistMatch ? '<br/><span style="opacity:.8">Watchlist match</span>' : ''}</div>` };
  ...
  public setSignalMapEvents(events: SignalMapEvent[]): void {
    this.signalMapEvents = events;
    this.render();
  }
