...
export class MapContainer {
  ...
  private cachedSignalMapEvents: SignalMapEvent[] | null = null;
  ...
  private rehydrateActiveMap(): void {
    ...
    if (this.cachedSignalMapEvents) this.setSignalMapEvents(this.cachedSignalMapEvents);
  ...
  public setSignalMapEvents(events: SignalMapEvent[]): void {
    this.cachedSignalMapEvents = events;
    const markerCount = events.filter((event) =>
    ...
    this.container.dataset.signalmapMarkerCount = String(markerCount);

    if (this.useGlobe) { this.globeMap?.setSignalMapEvents(events); return; }
    if (this.useDeckGL) {
      this.deckGLMap?.setSignalMapEvents(events);
    } else {
      this.svgMap?.setSignalMapEvents(events);
    }
  }
