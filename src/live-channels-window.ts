...
function customChannelIdFromStreamUrl(url: string): string {
  let slug = '';
  try {
  ...
  return `custom-stream-${slug || 'stream'}-${hashString(url)}`;
}
...
function appendStreamPath(url: URL, suffix: string): URL {
...
function withMutedEmbedParam(url: URL): string {
  const next = new URL(url.toString());
  if (!next.searchParams.has('initiallyMuted')) {
    next.searchParams.set('initiallyMuted', 'true');
  }
  return next.toString();
}

function parseCustomStreamInput(raw: string): CustomStreamInput | null {
  try {
    const url = new URL(raw);
    if (url.protocol !== 'https:' && url.protocol !== 'http:') return null;
    if (url.protocol === 'http:' && url.hostname !== '127.0.0.1' && url.hostname !== 'localhost') return null;
    if (parseYouTubeInput(raw)) return null;

    const nameHint = url.hostname.replace(/^www\./, '');
    if (url.pathname.endsWith('.m3u8') || raw.includes('.m3u8')) {
      let embedUrl: string | undefined;
      const hlsPath = url.pathname.match(/^(.*)\/hls\/[^/]+\.m3u8$/);
      if (hlsPath) {
        const embed = new URL(url.toString());
        embed.search = '';
        embed.hash = '';
        embed.pathname = `${hlsPath[1] || ''}/embed/video`;
        embedUrl = withMutedEmbedParam(embed);
      }
      return { sourceUrl: url.toString(), hlsUrl: url.toString(), embedUrl, nameHint };
    }

    if (url.pathname.replace(/\/+$/, '').endsWith('/embed/video')) {
      return { sourceUrl: url.toString(), embedUrl: withMutedEmbedParam(url), nameHint };
    }

    return {
      sourceUrl: url.toString(),
      hlsUrl: appendStreamPath(url, '/hls/stream.m3u8').toString(),
      embedUrl: withMutedEmbedParam(appendStreamPath(url, '/embed/video')),
      nameHint,
    };
  } catch {
    return null;
...
export async function initLiveChannelsWindow(containerEl?: HTMLElement): Promise<void> {
  ...
  function applyEditFormToChannels(
    ...
    if (isCustom) {
      const streamRaw = (formRow.querySelector('.live-news-manage-edit-stream') as HTMLInputElement | null)?.value?.trim();
      if (streamRaw) {
        const streamInput = parseCustomStreamInput(streamRaw);
        if (!streamInput) return null;
        const newId = customChannelIdFromStreamUrl(streamInput.sourceUrl);
        ...
        next[idx] = {
          ...
          videoId: undefined,
          hlsUrl: streamInput.hlsUrl,
          embedUrl: streamInput.embedUrl,
  ...
  function showEditForm(row: HTMLElement, ch: LiveChannel, listEl: HTMLElement): void {
    ...
    if (isCustom) {
      if (ch.hlsUrl || ch.embedUrl) {
        const streamInput = document.createElement('input');
        streamInput.type = 'text';
        streamInput.className = 'live-news-manage-edit-stream';
        streamInput.value = ch.hlsUrl ?? ch.embedUrl ?? '';
        streamInput.placeholder = t('components.liveNews.hlsUrl') ?? 'HLS or Owncast stream URL';
        ...
        <div class="live-news-manage-add-section">
          ...
          <div class="live-news-manage-add">
            <div class="live-news-manage-add-field">
              <label class="live-news-manage-add-label" for="liveChannelsHandle">${escapeHtml(t('components.liveNews.youtubeHandleOrUrl') ?? 'YouTube handle, video URL, or stream page')}</label>
              <input type="text" class="live-news-manage-handle" id="liveChannelsHandle" placeholder="@Channel, youtube.com/watch?v=..., or Owncast page URL" />
            </div>
            <div class="live-news-manage-add-field">
              <label class="live-news-manage-add-label" for="liveChannelsHlsUrl">${escapeHtml(t('components.liveNews.hlsUrl') ?? 'HLS or Owncast stream URL (optional)')}</label>
              <input type="text" class="live-news-manage-handle" id="liveChannelsHlsUrl" placeholder="https://example.com/hls/stream.m3u8 or Owncast page URL" />
            </div>
            <div class="live-news-manage-add-field">
  ...
  addBtn?.addEventListener('click', async () => {
    ...
    const addCustomStreamChannel = (streamInput: CustomStreamInput): boolean => {
      const id = customChannelIdFromStreamUrl(streamInput.sourceUrl);
      if (channels.some((c) => c.id === id)) return false;

      const name = nameInput?.value?.trim() || streamInput.nameHint || 'Custom Stream';
      channels.push({
        id,
        name,
        hlsUrl: streamInput.hlsUrl,
    ...
    // Check if custom HLS/Owncast URL is provided.
    if (hlsUrl) {
      const streamInput = parseCustomStreamInput(hlsUrl);
      if (!streamInput) {
        if (hlsInput) {
          hlsInput.classList.add('invalid');
          hlsInput.setAttribute('title', t('components.liveNews.invalidHlsUrl') ?? 'Enter a valid HLS stream URL or Owncast page URL');
        }
        return;
      }

      addCustomStreamChannel(streamInput);
      return;
    }
    ...
    const streamFromPrimary = parseCustomStreamInput(raw);
    if (streamFromPrimary) {
      addCustomStreamChannel(streamFromPrimary);
      return;
    }
