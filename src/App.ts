...
import { isSignalMapVariant } from '@/services/signalmap';
...
export class App {
  ...
  private async primeVisiblePanelData(forceAll = false): Promise<void> {
    if (isSignalMapVariant(SITE_VARIANT)) return;
  ...
  constructor(containerId: string) {
    ...
    if (isSignalMapVariant(SITE_VARIANT)) {
      document.documentElement.classList.add('variant-signalmap');
      document.documentElement.dataset.variant = 'signalmap';
    }
