...
// ============================================
// SIGNALMAP variant - compact event shell plus map
// ============================================
const SIGNALMAP_PANELS: Record<string, PanelConfig> = {
  map: { name: 'SignalMap Atlas', enabled: true, priority: 1 },
  signalmap: { name: 'SignalMap', enabled: true, priority: 1 },
};

const SIGNALMAP_MAP_LAYERS: MapLayers = {
  ...TECH_MAP_LAYERS,
  cables: true,
  outages: true,
  datacenters: true,
...
const SIGNALMAP_MOBILE_MAP_LAYERS: MapLayers = {
...
export const VARIANT_DEFAULTS: Record<string, string[]> = {
  ...
  signalmap: Object.keys(SIGNALMAP_PANELS),
};
...
export const VARIANT_PANEL_OVERRIDES: Partial<Record<string, Partial<Record<string, Partial<PanelConfig>>>>> = {
  ...
  signalmap: {
    map:       { name: 'SignalMap Atlas' },
    signalmap: { name: 'SignalMap' },
  },
};

/**
 * Returns the effective panel config for a given key and variant,
 * applying variant-specific display overrides.
 */
  ...
  : SITE_VARIANT === 'tech'
    ...
    : SITE_VARIANT === 'finance'
      ...
      : SITE_VARIANT === 'commodity'
        ...
        : SITE_VARIANT === 'energy'
          ? ENERGY_MAP_LAYERS
          : SITE_VARIANT === 'signalmap'
            ? SIGNALMAP_MAP_LAYERS
            : FULL_MAP_LAYERS;
