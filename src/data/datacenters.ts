// Ported verbatim from legacy worldmonitor src/config/ai-datacenters.ts.
// Source: Epoch AI GPU Clusters dataset (CC-BY).
// Filter: clusters with >1000 GPUs, Existing/Planned status, Confirmed/Likely certainty.
...
export type DataCenter = {
  ...
  lat: number;
  lon: number;
  status: 'operational' | 'planned' | 'under_construction' | string;
  chipType?: string;
  chipCount?: number;
  powerMW?: number;
};
...
// Top-30 by chipCount — used as the 'Main' subset, similar to FLAGSHIP_CABLE_IDS.
export const FLAGSHIP_DATACENTER_IDS = new Set<string>(
  [...DATA_CENTERS]
    .sort((a, b) => (b.chipCount ?? 0) - (a.chipCount ?? 0))
    .slice(0, 30)
    .map(d => d.id),
);
